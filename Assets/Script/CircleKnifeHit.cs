...
public class CircleKnifeHit : MonoBehaviour {
	...
	public static CircleKnifeHit instance;
	// Use this for initialization
	float valueZ ;
	void Start () {
		if (!isBoss) {
			GetComponent<SpriteRenderer>().sprite= GameManagerKnifeHit.Stage%20<10?woodSprite:blueWoodSprite;
		}
	...
	void SpawnApple()
	{
		foreach (float item in RandomLevels[currentLevelndex].AppleAngles) {
			GameObject tempApple = Instantiate<GameObject> (GamePlayManagerKnifeHit.instance.ApplePrefab);
	...
	void SpawnKnife()
	{
		foreach (float item in RandomLevels[currentLevelndex].KnifeAngles) {
			GameObject tempKnife = Instantiate<GameObject> (GamePlayManagerKnifeHit.instance.knifePrefab.gameObject);
	...
	public  void OnKnifeHit(Knife k){
		...
		if (hitedKnife.Count >= totalKnife) {
				if (!GameManagerKnifeHit.isGameOver) {
					StartCoroutine (RelaseAllKnife ());
					SoundManagerKnifeHit.instance.PlaySingle (LasthitSfx);
				}
			} else {
				playParticle(GamePlayManagerKnifeHit.instance.circleSpawnPoint.transform.position,splashParticle);
				SoundManagerKnifeHit.instance.PlaySingle (woodHitSfx);
			}
        GameManagerKnifeHit.score++;
			 if (GameManagerKnifeHit.score == 55){
            GameManagerKnifeHit.score *= 2; 
			 }
	...
	public IEnumerator RelaseAllKnife()
	{
		...
		if (!isBoss) {
			playParticle (transform.position, GameManagerKnifeHit.Stage % 20 < 10? WoodSplatParticle : BlueWoodSplatParticle);
		} else {
		...
		GamePlayManagerKnifeHit.instance.NextLevel ();
